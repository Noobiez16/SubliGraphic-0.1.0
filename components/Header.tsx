...
const Header: React.FC<HeaderProps> = ({ theme, cartItemCount, onCartClick }) => {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-[--color-surface] shadow-md">
            <nav className="px-4 sm:px-6 md:px-8 py-4 flex justify-between items-center">
                <div className="flex items-center space-x-2">
                    <svg className="h-8 w-8 text-[--color-primary]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span className={`text-2xl font-bold text-[--color-on-surface] ${isIOS ? 'text-ios-safe' : ''}`}>SubliGraphic</span>
                </div>
                <button onClick={onCartClick} aria-label={`View cart with ${cartItemCount} items`} className="relative h-10 w-10 rounded-full flex items-center justify-center transition-colors duration-200 bg-[--color-primary]/10 hover:bg-[--color-primary]/20">
